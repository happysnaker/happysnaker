...
> 💛 **Sponsorware + 开源支持：** 当前可赞助任务见下方 **Current sponsorware board**；[qq-ai-bot](https://github.com/happysnaker/qq-ai-bot) / [RDLeader](https://github.com/happysnaker/RDLeader) 都在公开推进。如果不确定该打赏、看证明、买评审还是资助具体 issue，先走 [10-second support router](https://happysnaker.github.io/support/#sponsor-router)；如果已经想支持一个具体结果，直接看 [Productized offer cards](https://happysnaker.github.io/support/#offer-cards)（`qq-ai-bot #26 arm64` / `RDLeader #27` / `Deploy read` / repo-name tip）；也欢迎 [扫码支持](https://happysnaker.github.io/support/)（¥9.9 起）或购买付费评审服务 ¥29.9–¥199。

> **Point-in-time proof snapshot (2026-07-12):** `qq-ai-bot` and `RDLeader` had **0 open CodeQL / Dependabot / secret-scanning alerts**, and the tracked `qq-ai-bot` CI / CodeQL / Docker publish / arm64 smoke plus `RDLeader` CI / CodeQL were green. Run `python3 scripts/check_github_status.py --summary` for the live status before quoting current state. Current concrete asks live at [support/#current-asks](https://happysnaker.github.io/support/#current-asks): `qq-ai-bot #26 arm64`, `RDLeader #27`, or `Deploy read` for bot / agent / infra repos that run but still look toy-like ([sample](https://happysnaker.github.io/review/deploy-read-sample/)); if you want proof before tipping or buying a review, start at [support/#proof-before-payment](https://happysnaker.github.io/support/#proof-before-payment), or use the [10-second support router](https://happysnaker.github.io/support/#sponsor-router) to choose Tip / Proof / Review / Fund.
...
> **Operator handoff:** [docs/operator-handoff.md](docs/operator-handoff.md) is the first-read state map for future GitHub takeover turns. Reproduce the current proof/support gate with `python3 scripts/run_profile_preflight.py --link-scope core --workers 8 --skip-external`; add `--json` for machine-readable preflight step status and nested JSON checker stdout. Use `python3 scripts/check_manual_blockers.py --json`, `python3 scripts/check_stable_profile_links.py --json`, `python3 scripts/check_gh_usage.py --json`, `python3 scripts/check_ci_workflow_contract.py --json`, `python3 scripts/check_preflight_json_contract.py --json`, and `python3 scripts/check_checker_catalog.py --json` for machine-readable blocker/proof-link/helper/CI-contract/preflight/checker state.
>
...
- **Sponsor prospect pipeline:** [docs/sponsor-prospect-pipeline.md](docs/sponsor-prospect-pipeline.md) — maps homelab testers, bot builders, curators, sponsors, and review customers to the right proof link, support note, and guardrail

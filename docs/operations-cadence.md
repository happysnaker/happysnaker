...
3. Run `python3 scripts/check_support_routes.py --json` to confirm funding/support/issue-contact files still include the 10-second support router and default fallback; then run `python3 scripts/check_public_links.py --timeout 6 --workers 8 --json` for the core support/proof links; use `--scope profile --workers 12 --json` before claiming broader profile-link coverage.

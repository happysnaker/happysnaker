...
- **SUPPORT** — support assets, templates, paid-review funnels, or checklists that reinforce the profile.
...
| [`review / deploy-read funnel`](https://happysnaker.github.io/review/) | Paid review / deploy-read conversion surface | Review page, deploy-read sample, support page, 10-second support router, mailto templates, and CI-backed `check_review_funnel.py` now keep `¥29.9` / `¥99` paths visible | Keep `Deploy read` and `support/#sponsor-router` visible from profile, sponsor packet, support files, and share kit; verify with `python3 scripts/check_review_funnel.py --site-root ../happysnaker.github.io --live --timeout 8 --json` |
| [`backend-engineer-checklist`](https://github.com/happysnaker/backend-engineer-checklist) | Backend interview / self-review checklist | Support content for profile; routes to frozen sponsor one-pager; has sponsorware / maintenance topics | Keep as supportive asset, not flagship; keep examples and coverage row fresh |
| [`system-design-checklist`](https://github.com/happysnaker/system-design-checklist) | Architecture / interview / design review checklist | Support content for profile; routes to frozen sponsor one-pager; has sponsorware / maintenance topics | Keep as supportive asset, not flagship; keep examples and coverage row fresh |
| [`production-readiness-checklist`](https://github.com/happysnaker/production-readiness-checklist) | Release / launch / on-call checklist | Aligns with reliability/backend positioning; routes to frozen sponsor one-pager; has sponsorware / maintenance topics | Consider linking from proof index if it gets examples; keep coverage row fresh |
| [`.github`](https://github.com/happysnaker/.github) | Default community health files | Enables default support/contribution/code-of-conduct and funding fallback; routes repos without local support files through `support/#sponsor-router` | Keep quiet; maintain default hygiene and support-router fallback only |

...
- Support route checker: [scripts/check_support_routes.py](../scripts/check_support_routes.py) — verifies profile, default `.github`, `qq-ai-bot`, and `RDLeader` funding/support/issue-contact files still route to proof-before-payment, current asks, and concrete payment notes; this now runs in profile CI.
- Repository metadata checker: [scripts/check_repo_metadata.py](../scripts/check_repo_metadata.py) — verifies the core profile, flagship, Pages, and default community-health repository homepages, topics, descriptions, visibility, and archive state; this now runs in profile CI.

...
- Profile preflight runner: [scripts/run_profile_preflight.py](../scripts/run_profile_preflight.py) — wraps the docs, status, support-route, metadata, sponsor-release, manual-blocker, public-link, and external-follow-up checks into one command; supports `--external-only` plus external `--action-class` filters.

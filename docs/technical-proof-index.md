...
- Profile preflight runner: [scripts/run_profile_preflight.py](../scripts/run_profile_preflight.py) — wraps public-docs, stable profile-link, GitHub CLI helper usage, share-kit, README badge, GitHub status, support-route, metadata, sponsor-release, sponsor-issue, paid-review funnel, operations-log, issue-label, manual-blocker, public-link, and external-follow-up checks into one command; supports `--json`, `--external-only`, external `--action-class` filters, `--external-summary`, and scheduled-review due gating.

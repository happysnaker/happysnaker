...
- External follow-up checker: [scripts/check_external_followups.py](../scripts/check_external_followups.py) — summarizes the tracked external PRs plus `qq-ai-bot#26` and `RDLeader#27` with machine-readable action classes, next-action guidance, materials, and prepared candidate comments, without posting comments; supports `--action-class` filtering, `--json`, and `--summary` output for scheduled review.

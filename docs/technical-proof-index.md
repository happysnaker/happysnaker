...
- Profile preflight runner: [scripts/run_profile_preflight.py](../scripts/run_profile_preflight.py) — wraps the docs, status, support-route, metadata, sponsor-release, manual-blocker, public-link, and external-follow-up checks into one command.
...
- External follow-up checker: [scripts/check_external_followups.py](../scripts/check_external_followups.py) — summarizes the tracked external PRs plus `qq-ai-bot#26` and `RDLeader#27` with machine-readable action classes and next-action guidance, without posting comments.

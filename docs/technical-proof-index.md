...
- Stable profile proof-link checker: [scripts/check_stable_profile_links.py](../scripts/check_stable_profile_links.py) — keeps hard-coded profile self-check run links confined to [flagship-status-snapshot.md](flagship-status-snapshot.md), while README/proof docs use stable workflow links.

...
- Profile preflight runner: [scripts/run_profile_preflight.py](../scripts/run_profile_preflight.py) — wraps the docs, badge, status, support-route, metadata, sponsor-release, manual-blocker, public-link, and external-follow-up checks into one command; supports `--external-only` plus external `--action-class` filters.
...
- External follow-up checker: [scripts/check_external_followups.py](../scripts/check_external_followups.py) — summarizes the tracked external PRs plus `qq-ai-bot#26` and `RDLeader#27` with machine-readable action classes and next-action guidance, without posting comments; supports `--action-class` filtering for scheduled review.
...
- Public site hygiene verifier: [scripts/check_site_hygiene.py](../scripts/check_site_hygiene.py) ([commit](https://github.com/happysnaker/happysnaker/commit/ef88985863a24042f5f8be35c0b2979e44441d71)); latest live run checked 154 site hygiene assertions across project-page metadata, sitemap, public repo-link hygiene, and support-page proof-before-payment content.

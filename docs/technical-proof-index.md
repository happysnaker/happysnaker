...
- Profile preflight runner: [scripts/run_profile_preflight.py](../scripts/run_profile_preflight.py) — wraps public-docs, stable profile-link, GitHub CLI helper usage, checker catalog JSON, share-kit, sponsor prospect pipeline, README badge, GitHub status, support-route, metadata, sponsor-release, sponsor-issue, paid-review funnel, operations-log, issue-label, manual-blocker, public-link, and external-follow-up checks into one command; supports `--json`, `--external-only`, external `--action-class` filters, `--external-summary`, `--external-candidate-comments`, and scheduled-review due gating.
- README badge checker: [scripts/check_readme_badges.py](../scripts/check_readme_badges.py) — verifies first-screen CI / CodeQL badges, remote workflow-file existence for profile / `qq-ai-bot` / `RDLeader`, and first-screen support/proof routes to proof-before-payment, current asks, sponsor router, sponsor prospect pipeline, and deploy-read sample; supports `--json` for machine-readable first-screen badge/support state.

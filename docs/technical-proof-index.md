...
- Checker catalog checker: [scripts/check_checker_catalog.py](../scripts/check_checker_catalog.py) — verifies this proof index documents every `scripts/check_*.py` proof checker, including the catalog checker itself, so new checks do not become invisible; supports `--json` for machine-readable handoff state.
